...
void chargerTextures()
{
    ...
    {
        glGenTextures( 1, &Etat::textureMarioSMW );
        glBindTexture( GL_TEXTURE_2D, Etat::textureMarioSMW );
        glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, largeur, hauteur, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
        //glGenerateMipmap( GL_TEXTURE_2D );
        glBindTexture( GL_TEXTURE_2D, 0 );
...
void afficherModele()
{
    ...
    case 4: glBindTexture( GL_TEXTURE_2D, Etat::textureMarioSMW ); break;
    }
...
void FenetreTP::clavier( TP_touche touche )
{
    ...
    case TP_t: // Changer la texture utilisée: 0-aucune, 1-oiseau, 2-flocon
        if ( ++Etat::texnumero > 4 ) Etat::texnumero = 0;
